...
def guess_url(paper):
    ...
    if not url:
        url = "http://semdial.org/{}-{}".format(volume.attrib['id'], paper.attrib['id'])
...
def merge_people(variants, can1, can2):
    ...
    if i1 is not None and i2 is not None:
        logger.error("Please manually merge '{}' and '{}' in name_variants.yaml".format(can1, can2))
        return
    elif i1 is not None:
        i = i1
        new = can2
    ...
    for v in variants[i].get('variants', []):
        ...
        if var == new:
            return
    variants[i].setdefault('variants', []).append({'first': new.first, 'last': new.last}) # don't use to_dict because that adds extra fields
    ...
    for infile in infiles:
        ...
        for paper in volume.findall('paper'):
            ...
            for authornode in paper.xpath('./author|./editor'):
                ...
                person = anth.people.resolve_name(anthology.people.PersonName(first, last))['id']
                newperson = anth.people.resolve_name(anthology.people.PersonName(newfirst, newlast))['id']

                merge_people(variants,
