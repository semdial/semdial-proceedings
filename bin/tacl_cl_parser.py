...
STANDARD_URL = "http://semdial.org/anthology/{volume}-{paper}"
...
def get_article_journal_info(xml_front_node: etree.Element, is_tacl: bool) -> str:
    ...
    # Fixes
    journal_title_text = " ".join(journal_title_text.split())  # Sometimes it's split onto two lines...
    journal_title_text = journal_title_text.replace(  # Somebody in 2018 didn't know our name?
        "Association of Computational Linguistics",
        "Association for Computational Linguistics"
    )
    volume_text = volume.text.lstrip("0")  # Somebody brilliant decided that 2018 would be "06" instead of "6"
    ...
    data = dict(journal=journal_title_text, volume=volume_text, issue=issue_text, date=string_date_text)
...
def issue_info_to_node(issue_info: str, year_: str, volume_id: str, issue_counter: int, is_tacl: bool) -> etree.Element:
    ...
    if not is_tacl:
        ...
        assert month_text in {'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'}
        month = etree.Element('month')
        month.text = month_text
        node.append(month)
...
if __name__ == '__main__':
    ...
    for xml in sorted(args.year_root.glob("*_a_*/*.xml")):
        ...
        if issue_info != previous_issue_info:
            ...
            volume.append(issue_info_to_node(issue_info, year, volume_id, issue_count, is_tacl))
        ...
        if args.old_version:
            ...
            if old_paper is None:
                log.error(f"No old version for {paper_id} with title {papernode.find('title').text}")
            else:
